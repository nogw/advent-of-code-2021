  ...
  let rec aux = (scanners, fixed) =>
    ...
    | [hd, ...tl] => {
        ...
        if (List.length(fix) == 0) {
          aux(scanners, fixed, tl @ [hd]);
        } else {
          aux([s, ...scanners], fixed @ [fix], tl);
        };
      };
  let scanners = aux([(0, 0, 0)], [List.hd(l)], List.tl(l));

  scanners
  |> List.concat_map(x => List.map(y => (x, y), scanners))
  |> List.map((((x1, y1, z1), (x2, y2, z2))) =>
       (abs(x1 - x2), abs(y2 - y1), abs(z2 - z1))
     )
